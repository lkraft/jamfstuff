...
AD_DOMAIN="fcusa.local"
COMPUTER_ID=`/usr/sbin/scutil --get LocalHostName`
COMPUTER_SERIAL=`ioreg -l | grep IOPlatformSerialNumber|awk '{print $4}' | cut -d \" -f 2`
COMPUTERS_OU="OU=macintosh,OU=Computers,OU=FCUSA,DC=fcusa,DC=local"
ADMIN_LOGIN="svc-bind"
ADMIN_PWD=")AZU797+9&W;|6G/z\k"
MOBILE="enable"
...
PASSWORD_INTERVAL="0"
ADMIN_GROUPS="FCUSA\Domain Admins,FCUSA\Enterprise Admins"
...
# Check if Computer name is the Serial Number of the mac
if [ ${COMPUTER_ID} -eq ${COMPUTER_SERIAL} ]
then
echo "The mac name is not it serial, will fix!"
  scutil --set LocalHostName "$COMPUTER_SERIAL"
  scutil --set ComputerName "$COMPUTER_SERIAL"
  scutil --set HostName "$COMPUTER_SERIAL"
fi
